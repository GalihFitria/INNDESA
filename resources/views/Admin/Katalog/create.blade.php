...
<div class="bg-white shadow-md p-4 rounded-lg max-w-2xl mx-auto">
    <form action="{{ route('Admin.katalog.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
        ...
        <div>
            ...
            <input type="file" name="katalog" id="katalog" accept=".pdf,.jpg,.jpeg,.png"
    ...
    <div id="previewModal"
        ...
        <div class="bg-white p-4 rounded-lg w-[800px] h-[600px] relative">
            <button onclick="closePreview()"
                class="absolute top-2 right-2 bg-red-500 text-white px-3 py-1 rounded">✕</button>
...
<script>
    ...
    function previewFile(file) {
        ...
        if (file.type.startsWith('image/')) {
            ...
            previewControls.classList.remove('hidden'); // hanya muncul kalau image
    ...
    function rotateImage(degree) {
        ...
        if (!cropper) {
            ...
            img.onload = () => {
                ...
                canvas.toBlob((blob) => {
                    if (blob) {
                        katalogFile = new File([blob], katalogFile ? katalogFile.name : 'image.jpg', {
                            type: 'image/jpeg'
                        });
                        updateKatalogPreview();
                    }
