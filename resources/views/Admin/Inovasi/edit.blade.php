...
<div class="bg-white shadow-md p-4 rounded-lg max-w-2xl mx-auto">
    <form action="{{ route('Admin.inovasi.update', $inovasi->id_inovasi) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            @if ($inovasi->foto)
            <div class="mb-2">
                <a href="#" onclick="event.preventDefault(); previewExistingFile('{{ asset('storage/' . $inovasi->foto) }}', '{{ str_ends_with($inovasi->foto, '.pdf') ? 'application/pdf' : 'image/' }}')" class="text-blue-600 hover:underline">
                    {{ basename($inovasi->foto) }}
                </a>
            </div>
            @endif
