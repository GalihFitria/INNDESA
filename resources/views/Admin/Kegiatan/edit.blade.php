...
<div class="bg-white shadow-md p-4 rounded-lg max-w-2xl mx-auto">
    <form action="{{ route('Admin.kegiatan.update', $kegiatan->id_kegiatan) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            @if ($kegiatan->foto)
            <div class="mb-2">
                <a href="#" onclick="event.preventDefault(); previewExistingFile('{{ asset('storage/' . $kegiatan->foto) }}', '{{ str_ends_with($kegiatan->foto, '.pdf') ? 'application/pdf' : 'image/' }}')" class="text-blue-600 hover:underline">{{ basename($kegiatan->foto) }}</a>
            </div>
            @endif
...
<script>
    ...
    function previewFile(file) {
        ...
        rotatedImageUrl = originalImageUrl; // Initialize with original

        previewImage.classList.add('hidden');
        previewFrame.classList.add('hidden');
        cropperControls.classList.add('hidden');
        previewControls.classList.remove('hidden'); // Show preview controls initially
        previewImage.src = '';
        previewFrame.src = '';
        ...
        if (file.type.startsWith('image/')) {
            ...
            previewControls.classList.add('hidden'); // Hide controls for PDF
        } else {
    ...
    function rotateImage(degree) {
        const previewImage = document.getElementById('previewImage');
        if (!cropper) {
            // Rotate the image using a canvas if not in crop mode
            ...
            img.onload = () => {
                ...
                previewImage.src = rotatedImageUrl;
                // Update fotoFile with the rotated image
                canvas.toBlob((blob) => {
                    if (blob) {
            ...
            // Rotate using cropper if in crop mode
    ...
    function cancelCrop() {
        if (cropper) {
            ...
            cropper = null;
            const previewImage = document.getElementById('previewImage');
            previewImage.src = rotatedImageUrl; // Use the last rotated state
            document.getElementById('cropperControls').classList.add('hidden');
            document.getElementById('previewControls').classList.remove('hidden'); // Show preview controls again
            isCropping = false;
        }
    }
